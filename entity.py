...
def grabAuthorInfo(link):
    headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36'}
    page = requests.get(link, headers=headers)
    tree = html.fromstring(page.content)
    author = tree.xpath('//span[@class="tiktok-lh6ok5-SpanOtherInfos e17fzhrb2"]/text()')
    date = tree.xpath('//span[@class="tiktok-lh6ok5-SpanOtherInfos e17fzhrb2"]/span/text()')
    imgLink = tree.xpath('//img[@class="tiktok-1zpj2q-ImgAvatar e1e9er4e1"]/@src')
    authorHandle = tree.xpath('//a[@class="tiktok-19x2iw0 e17fzhrb5"]/@href')
    for obj in date:
        author.append(obj)
    authLink = "https://www.tiktok.com"+authorHandle[0]
    pfpLink = ""
    author.append(authLink)
    author.append(authorHandle[0].replace("/",""))
    author.append(imgLink[0])
    return author
...
def grabVideo(link):
    ...
    driver.get(link)
    videoLink = driver.find_elements(By.XPATH, '//div[@class="tiktok-1h63bmc-DivBasicPlayerWrapper e1yey0rl2"]/div/video')
    for obj in videoLink:
        link = obj.get_attribute("src")
...
async def birthday(ctx, bday=None):
    file = open('./files/birthdays.json', 'r+')
    data = json.load(file)
...
async def services(ctx):
    ...
    embedMsg.add_field(name="The Entity Bot", value="Me! I am a continuously upgraded passion project maintained by RJ.", inline=True)
...
async def on_message(msg):
    ...
    if "https://www.tiktok.com/" in raw:
        ...
        try:
            ...
            if(exists("./out.mp4")):
                os.remove("./out.mp4")
            author = grabAuthorInfo(url)
            print(url)
            grabVideo(url)

            tiktokEmbed.set_author(name=str(author[0]+" ("+author[4]+")"),url=author[3],icon_url=author[5])
            tiktokEmbed.set_footer(text="TikTok "+author[1]+" "+author[2], icon_url="https://cdn4.iconfinder.com/data/icons/social-media-flat-7/64/Social-media_Tiktok-512.png")
            with open('out.mp4','rb') as fp:
                await msg.edit(suppress=True)
                await msg.channel.send(embed=tiktokEmbed)
                await msg.channel.send(file=discord.File(fp, 'tiktok.mp4'))
            os.remove("./out.mp4")
        except IndexError:
